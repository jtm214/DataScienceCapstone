...
kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
              ...
              full_width = TRUE)
```
Now we're going to go into a little more depth on the data.  First, the timestamp of the rating information is going to grabbed from the data sets along with the year the movie was release.  The time information is also going to parsed out of the final_holdout_test data set so that it can be used in the final analysis if needed.
...
Now we're going to add the delta between the year the movie was released and the year that the movie was rated effects.  The assumption is that the longer the delta is the higher the rating will be, kind of a nastalgia effect.  
